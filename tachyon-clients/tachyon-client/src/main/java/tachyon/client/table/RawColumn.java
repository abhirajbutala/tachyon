...
public class RawColumn {
  ...
  private static final String COL = "COL_";
  ...
  public boolean createPartition(int pId) throws IOException {
    TachyonURI tUri =
        new TachyonURI(CommonUtils.concat(mRawTable.getPath(), COL + mColumnIndex, pId));
  ...
  public int partitions() throws IOException {
    TachyonURI tUri =
        new TachyonURI(CommonUtils.concat(mRawTable.getPath(), COL + mColumnIndex));
